...
<div class="min-h-screen bg-slate-950 py-8 px-4 sm:px-6 lg:px-8 text-slate-200">
    <div class="max-w-5xl mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-6">
            <div>
                <h2 class="text-3xl font-bold text-white tracking-tight">Edit Category</h2>
                <p class="mt-1 text-sm text-slate-400">Update the category name and color used across the inventory.</p>
            </div>
            ...
            <div class="grid gap-8 lg:grid-cols-[1.4fr_0.9fr] p-8">
                <div class="space-y-6">
                    <div class="rounded-3xl bg-slate-950/60 border border-slate-800 p-6">
                        <h3 class="text-xl font-semibold text-white">Category settings</h3>
                        <p class="mt-2 text-sm text-slate-400">Edit the label that appears throughout your inventory dashboard.</p>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                ...
                                <div class="flex h-14 w-14 items-center justify-center rounded-3xl border border-slate-700 bg-slate-900">
                                    <div class="h-10 w-10 rounded-full" style="background-color: {{ old('cat_color', $category->cat_color) }}"></div>
                                </div>
                            </div>
                            <p class="text-xs text-slate-500 mt-2">This color is used for category badges and labels.</p>
                            @error('cat_color')
                                <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="space-y-6">
                    <div class="rounded-3xl border border-slate-800 bg-slate-950/40 p-6">
                        <h3 class="text-lg font-semibold text-white">Update tips</h3>
                        <ul class="mt-4 space-y-3 text-sm text-slate-400">
                            <li class="flex gap-3 items-start">
                                <span class="mt-1 inline-flex h-2.5 w-2.5 rounded-full bg-indigo-400"></span>
                                Use a short category name for easy scanning.
                            </li>
                            <li class="flex gap-3 items-start">
                                <span class="mt-1 inline-flex h-2.5 w-2.5 rounded-full bg-indigo-400"></span>
                                Choose a clear color so the badge stands out in lists.
                            </li>
                        </ul>
                    </div>
        ...
        <div class="mt-6 text-center text-xs text-slate-500 italic">Update category settings used across the inventory.</div>
    </div>
</div>
@endsection
